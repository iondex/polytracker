...
class DataFlowSanitizer : public ModulePass {
	...
	FunctionType *DFSanBlockFnTy;
	Constant *DFSanBlockFn;

	//Function to log entry,exits,longjmps
	...
	std::string getBasicBlockName(BasicBlock * bb);
...
bool DataFlowSanitizer::doInitialization(Module &M) {
	...
	Type * DFSanBlockArgs[1] = {Type::getInt8PtrTy(*Ctx)};
	DFSanBlockFnTy = FunctionType::get(Type::getVoidTy(*Ctx), DFSanBlockArgs, false);
...
//This returns a blocks name if it has one, or captures the printer output
std::string DataFlowSanitizer::getBasicBlockName(BasicBlock * bb) {
	if (!bb->getName().empty()) {
...
bool DataFlowSanitizer::runOnModule(Module &M) {
	...
	DFSanExitFn =
			Mod->getOrInsertFunction("__dfsan_func_exit", DFSanExitFnTy);
	DFSanResetFrameFn =
			Mod->getOrInsertFunction("__dfsan_reset_frame", DFSanResetFrameFnTy);
	DFSanBlockFn =
			Mod->getOrInsertFunction("__dfsan_block_entry", DFSanBlockFnTy);
	...
	std::unordered_map<std::string, int> function_nonce_map;
    ...
    auto store_inst = IRB.CreateStore(
        IRB.CreateCall(DFSanEntryFn, FuncName),
        FrameIndex);
    if (function_nonce_map.find(i->getName().str()) != function_nonce_map.end()) {
    	function_nonce_map[i->getName().str()] = 0;
    }
    //Get and increment nonce
    int block_val = function_nonce_map[i->getName().str()];
    function_nonce_map[i->getName().str()] += 1;
    Value * BlockName = IRB.CreateGlobalStringPtr(std::to_string(block_val));
    //Create call to log the entry block
    IRB.SetInsertPoint(store_inst->getNextNode());
    IRB.CreateCall(DFSanBlockFn, BlockName);
		...
		//FIXME We can merge this into one of the other loops if it works okay
		for (BasicBlock * curr_bb : BBList) {
			//If we are at the start of the function, add the store, then the call after
		    int block_num = function_nonce_map[i->getName().str()];
			std::string curr_block_name = std::to_string(block_num);
			std::string fname = i->getName();
			if (curr_bb == BB) {
				std::cout << "FUNC ENTRY: " << fname << " SKIPPING BLOCK: " << curr_block_name << std::endl;
			}
			//Else this is just another basic block we need to track
			else {
			    function_nonce_map[i->getName().str()] += 1;
				std::cout << "FUNC: " << fname << "BASIC BLOCK: " << curr_block_name <<  std::endl;
				IRB.SetInsertPoint(&(*(curr_bb->getFirstInsertionPt())));
			    Value * CurrBlockName = IRB.CreateGlobalStringPtr(getBasicBlockName(curr_bb));
			    IRB.CreateCall(DFSanBlockFn, BlockName);
			}
		}
		//FIXME keep this for debugging, but this is to make sure we dont dedup instrumentation on blocks
		std::unordered_map<BasicBlock *, bool> previously_seen;
		for (BasicBlock * curr_bb : BBList) {
			if (previously_seen.find((BasicBlock*)curr_bb) != previously_seen.end()) {
				std::cout << "ERROR: IDENTICAL BLOCKS IN THIS LOST, WILL HAVE DOUBLE INST" << std::endl;
			}
			else {
				previously_seen[curr_bb] = true;
			}
		}
...
static void registerAflDFSanPass(const PassManagerBuilder&, legacy::PassManagerBase &PM) {
